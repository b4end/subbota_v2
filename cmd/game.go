...
func (g *Game) Update() error {
	...
	// Переменная g.ticks хранит время жизни игры. Обычно используется для
	// анимаций, зависящих от времени (например, чтобы меч плавно покачивался,
	// вычисляя синус от этого числа), даже если сейчас в коде это закомментировано.
	g.ticks++

	// ----------------------------------------------------------------------
	// 1. ОПРЕДЕЛЯЕМ ЦЕЛЕВОЙ УГОЛ (Идеальное положение, куда меч должен прийти)
	// ----------------------------------------------------------------------

	// Создаем локальную переменную destAngle (Destination Angle - конечный угол).
	// Она будет хранить угол в радианах (от 0 до 2*Пи), где меч должен находиться
	// в ИДЕАЛЕ прямо сейчас, в зависимости от действий игрока.
	destAngle := 0.0
	...
	if g.playerVY > 0 {
		// playerVY > 0 означает, что игрок ПАДАЕТ вниз.
		// Мы прибавляем к углу 0.4 радиана. Так как Y растет вниз, увеличение угла
		// смещает целевую точку по окружности вниз. Меч как бы "задирается" из-за сопротивления воздуха.
		destAngle += 0.4
	} else if g.playerVY < 0 {
		// playerVY < 0 означает, что игрок ЛЕТИТ ВВЕРХ (прыгнул).
		// Мы отнимаем 0.4 радиана, смещая целевую точку вверх. Меч "прижимается" вниз.
		destAngle -= 0.4
	}

	// ----------------------------------------------------------------------
	// 2. ПЛАВНЫЙ ПЕРЕЛЕТ ЦЕЛЕВОГО УГЛА (Виртуальная "бегущая" цель)
	// ----------------------------------------------------------------------

	// angleSpeed определяет, какую долю пути (10%) виртуальная цель меча
	// проходит за один кадр. Это делает перелет с левого плеча на правое плавным.
	const angleSpeed = 0.1

	// diff (Difference) - это разница между тем, где мы ХОТИМ быть (destAngle),
	// и тем, куда наша виртуальная цель стремится СЕЙЧАС (g.sword.targetAngle).
	diff := destAngle - g.sword.targetAngle

	// ДВА ЦИКЛА НИЖЕ - ЭТО НОРМАЛИЗАЦИЯ УГЛА.
	// Окружность - это 360 градусов (2*Пи). Если меч на 350 градусах, а цель на 10,
	// математически разница = -340. Если просто прибавить, меч прокрутится почти на полный круг назад.
	// Эти циклы заставляют меч выбрать КРАТЧАЙШИЙ путь (разница станет +20 градусов).
	for diff < -math.Pi {
		diff += 2 * math.Pi
	}
	for diff > math.Pi {
	...
	// Применяем Линейную Интерполяцию (Lerp). Сдвигаем текущий виртуальный угол
	// на небольшой шаг (diff * 0.1) в сторону идеального угла.
	g.sword.targetAngle += diff * angleSpeed

	// ----------------------------------------------------------------------
	// 3. ВЫЧИСЛЯЕМ "ИДЕАЛЬНУЮ" ЦЕЛЬ (Декартовы координаты на чистой окружности)
	// ----------------------------------------------------------------------

	// Задаем базовый радиус орбиты меча. SRR - коэффициент масштабирования экрана,
	// чтобы орбита была одинаковой визуально на любых мониторах.
	baseRadius := 18.0 * SRR

	// Находим точную координату X и Y центра спрайта игрока.
	// g.playerX - это левый верхний край картинки, прибавляем половину ширины (PlayerW/2), получаем центр.
	...
	// Формула: X = ЦентрX + Косинус(угол) * Радиус
	// Формула: Y = ЦентрY + Синус(угол) * Радиус
	// Это дает нам точку (targetX, targetY), которая плавно бежит по ИДЕАЛЬНО круглой орбите вокруг игрока.
	targetX := centerX + math.Cos(g.sword.targetAngle)*baseRadius
	targetY := centerY + math.Sin(g.sword.targetAngle)*baseRadius

	// ----------------------------------------------------------------------
	// 4. ВЯЗКАЯ ФИЗИКА (Реальный меч пытается догнать виртуальную точку)
	// ----------------------------------------------------------------------

	// followSpeed - скорость, с которой физические координаты меча тянутся к целевой точке.
	const followSpeed = 0.2

	// Применяем Lerp для физической позиции меча по оси X.
	// (targetX - g.sword.x) - это расстояние от меча до цели по X. Мы берем от него 20%
	// и прибавляем к текущей позиции. Меч движется как на резиновой нити.
	g.sword.x += (targetX - g.sword.x) * followSpeed

	// Делаем то же самое для Y, но делим followSpeed на 3 (около 0.066).
	// Это гениальный трюк: по вертикали меч "догоняет" цель в 3 раза медленнее.
	// Это создает ощущение тяжести и пружинистости при прыжках.
	g.sword.y += (targetY - g.sword.y) * followSpeed / 3

	// ВАЖНО: Из-за того, что меч догоняет цель "срезая углы" (двигаясь по прямой
	// между своей позицией и бегущей целью), он проваливается внутрь орбиты
	// (внутрь текстуры игрока). Следующий шаг исправляет это.

	// ----------------------------------------------------------------------
	// 5. ВОЗВРАТ НА ОРБИТУ И ВЫТАЛКИВАНИЕ (Проекция с эллиптическим сплющиванием)
	// ----------------------------------------------------------------------

	// math.Atan2 - это мощная функция, которая берет разницу по Y и разницу по X
	// и возвращает реальный угол между центром игрока и ТЕКУЩЕЙ физической позицией меча.
	// То есть мы спрашиваем: "Окей, меч срезал угол, но на каком градусе относительно центра он сейчас находится?"
	currentAngle := math.Atan2(g.sword.y-centerY, g.sword.x-centerX)

	// Высчитываем коэффициент сплющивания (pinch) в зависимости от того, где сейчас меч.
	// math.Sin(currentAngle) дает: 0 на боках (лево/право), и 1 (или -1) строго сверху/снизу.
	// math.Abs делает значение всегда положительным (от 0.0 до 1.0).
	pinch := math.Abs(math.Sin(currentAngle))

	// Формируем динамический радиус.
	// Если меч сбоку (pinch = 0), радиус = baseRadius * 1.0 (полный радиус).
	// Если меч сверху (pinch = 1), радиус = baseRadius * 0.75 (уменьшен на 25%).
	// Это превращает орбиту меча из идеального круга в сплюснутый эллипс (дыню).
	...
	// Мы берем центр игрока, направляем вектор по текущему углу меча и "выдвигаем"
	// меч строго на длину currentRadius. Теперь меч не может провалиться внутрь игрока,
	// он всегда скользит строго по границе нашего невидимого эллипса.
	g.sword.x = centerX + math.Cos(currentAngle)*currentRadius
	g.sword.y = centerY + math.Sin(currentAngle)*currentRadius

	// ----------------------------------------------------------------------
	// 6. ПОВОРОТ САМОГО МЕЧА (Визуальная ротация спрайта)
	// ----------------------------------------------------------------------

	// Записываем финальный угол в структуру меча.
	// В методе Draw функция op.GeoM.Rotate(g.sword.angle) повернет картинку меча так,
	// чтобы она смотрела точно от центра игрока наружу (как спица в колесе).
